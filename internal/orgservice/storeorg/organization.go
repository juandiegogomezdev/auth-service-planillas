...
func (s *store) GetAllUserOrganizations(userID uuid.UUID) ([]dtoorg.CreatedOrganization, error) {
	q := `
		SELECT id, owner_user_id, name, type, created_at
		FROM organizations
		WHERE owner_user_id=$1
	`
	var orgs []dtoorg.CreatedOrganization
	if err := s.db.Select(&orgs, q, userID); err != nil {
		return nil, fmt.Errorf("error fetching user organizations: %w", err)
	}
	return orgs, nil
}

func (s *store) ExistOrganizationPersonal(userID uuid.UUID) (bool, error) {
	qCheck := `SELECT EXISTS(SELECT 1 FROM organizations WHERE owner_user_id=$1 AND type=$2)`
	var exists bool
	if err := s.db.Get(&exists, qCheck, userID, "personal"); err != nil {
